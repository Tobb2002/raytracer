...
int main(void) {
  ...
  vec3 origin = vec3(-6, 0, -10);
  vec3 origin_plane = vec3(0, 0, -5);
  //Mesh m = Mesh("data/input/skull.obj",
  //    vec3(0, 15, -20), {.color = vec3(0, 1, 0)});
  Mesh m = Mesh("data/input/bunny_smooth.obj",
      origin_plane + vec3(0, -1, -10), {.color = vec3(0, 1, 0), .specular=0.2});
  ...
  Pointlight light = Pointlight(origin + vec3(0, 2, 4), 300);
  Pointlight light1 = Pointlight(origin, 200);
  ...
  //scene.get_camera()->move(vec3(0, 10, 0));
